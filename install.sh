...
function usage()
{
   ...
   optional arguments:
     ...
     -c, --collection COLLECTION  install a specific openvox-agent collection (e.g. openvox7)
...
# This script may be called outside of a task, e.g. by openvox_agent
...
maybe_delegate_to_facter() {
  ...
  # used by the openvox_agent task.
...
if [ -n "$PT_mac_source" ]; then
  ...
  #if [ "$nightly" = true ]; then
  #  mac_source='http://nightlies.openvox.com/downloads'
  #else
    mac_source='https://downloads.voxpupuli.org/mac'
  #fi
fi
...
if [ -z "$version" ]; then
  ...
  if [ "$version" == "$installed_version" ]; then
    ...
    openvox_agent_version="$version"
  fi
fi

# Error if non-root
if [ `id -u` -ne 0 ]; then
  echo "openvox_agent::install task must be run as root"
  exit 1
fi
  ...
  case "$1" in
    "rpm")
      ...
      if test "$version" = 'latest'; then
        ...
        run_cmd "${PKGCMD} install -y 'openvox-agent-${openvox_agent_version}'"
      fi
      ;;
    "noarch.rpm")
      ...
      if test "$version" = "latest"; then
        ...
        run_cmd "zypper install --no-confirm --oldpackage --no-recommends --no-confirm 'openvox-agent-${openvox_agent_version}'"
      fi
      ;;
    "deb")
      ...
      if test "$version" = 'latest'; then
        ...
        if test "x$deb_codename" != "x"; then
          run_cmd "${frontend} apt-get install -y 'openvox-agent=${openvox_agent_version}-1${deb_codename}'"
        else
          run_cmd "${frontend} apt-get install -y 'openvox-agent=${openvox_agent_version}'"
        fi
      fi
      ;;
    "dmg" )
...
info "Downloading openvox $version for ${platform}..."
case $platform in
  "SLES")
    ...
    if [[ $PT__noop != true ]]; then
      for key in "openvox"; do
        ...
        do_download "https://yum.voxpupuli.org/GPG-KEY-${key}.pub" "$gpg_key"
        rpm --import "$gpg_key"
        rm -f "$gpg_key"
      done
    fi
